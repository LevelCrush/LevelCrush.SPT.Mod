...
class InstantRepeatables implements IPreAkiLoadMod {
  ...
  preAkiLoad(container: DependencyContainer): void {
    ...
    container.register<QuestController>(
      "QuestControllerOriginal",
      QuestController
    );
    container.register<CustomRepeatableQuestController>(
      "CustomRepeatableQuestController",
      CustomRepeatableQuestController
    );
    container.register<RepeatableQuestController>("RepeatableQuestController", {
      useToken: "CustomRepeatableQuestController",
    });
  }

  refreshDailyOrWeekly(
    pmcData: IPmcData,
    body: ICompleteQuestRequestData,
    sessionID: string
  ): IItemEventRouterResponse {
    const questController =
      InstantRepeatables.container.resolve<QuestController>(
        "QuestControllerOriginal"
      );

    const configServer =
      InstantRepeatables.container.resolve<ConfigServer>("ConfigServer");
    const jsonUtil: JsonUtil =
      InstantRepeatables.container.resolve<JsonUtil>("JsonUtil");
    const questConfig: IQuestConfig = configServer.getConfig(ConfigTypes.QUEST);

    const repeatableQuestController =
      InstantRepeatables.container.resolve<CustomRepeatableQuestController>(
        "CustomRepeatableQuestController"
      );
    ...
    for (const currentRepeatable of pmcData.RepeatableQuests) {
      ...
      const questTypePool = repeatableQuestController.generateQuestPoolPublic(
        repeatableConfig,
        pmcData.Info.Level
      );
    ...
    if (!repeatableToChange) return output;
