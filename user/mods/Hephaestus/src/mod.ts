...
class Hephaestus implements IPreAkiLoadMod, IPostDBLoadMod {
    ...
    public postDBLoad(container: DependencyContainer): void {
        ...
        const importer = container.resolve<ImporterUtil.ImporterUtil>('ImporterUtil');
        ...
        for (const locale of locales) {
            ...
            locale[`${baseJson._id} Description`] =
                'You share the reseller license of your creations to Hephaestus and in return you get a hefty discount.';
        }
    ...
    private resetQuests(container: DependencyContainer, sessionId: string) {
        let profile = container.resolve<ProfileHelper>('ProfileHelper').getFullProfile(sessionId);
    ...
    private getPresets(container: DependencyContainer, assortTable, currency, profiles) {
        ...
        for (let p in profiles || []) {
            ...
            for (let wb of profiles[p].userbuilds.weaponBuilds) {
                ...
                try {
                    price = ragfairPriceService.getDynamicOfferPriceForOffer(preItems, currency, false);
                } catch (error) {}
                if (config?.discount) {
                ...
                try {
                    ...
                    for (let i = 0; i <= preItems.length; i++) {
                        ...
                        if (item.parentId == masterId) {
                            ...
                            preItems
                                .filter((pr) => (pr || {}).parentId == item._id)
                                .forEach((pr) => {
                                    reward.items.push({
                                        _id: pr._id,
                                        _tpl: pr._tpl,
                                    ...
                                    try {
                                        preItems
                                            .filter((prr) => prr.parentId == pr._id)
                                            .forEach((prr) => {
                                                reward.items.push({
                                                    _id: prr._id,
                                                    _tpl: prr._tpl,
                                                ...
                                                preItems
                                                    .filter((prrr) => prrr.parentId == prr._id)
                                                    .forEach((prrr) => {
                                                        reward.items.push({
                                                            _id: prrr._id,
                                                            _tpl: prrr._tpl,
                                                        ...
                                                        preItems
                                                            .filter((prrrr) => prrrr.parentId == prrr._id)
                                                            .forEach((prrrr) => {
                                                                reward.items.push({
                                                                    _id: prrrr._id,
                                                                    _tpl: prrrr._tpl,
    ...
    private createAssortTable(container, sessionId): ITraderAssort {
        ...
        try {
            assortTable = this.getPresets(
                container,
                assortTable,
                config.currency || '569668774bdc2da2298b4568',
                profiles,
            );
        } catch (error) {
            // console.error(error);
