...
class Mod implements IPreAkiLoadMod {
    ...
    staticRouterModService.registerStaticRouter(
      ...
      [
        ...
        },
        {
          ...
          //output is the response back to client
          action: (url, info, sessionID, output) => {
            let response = this.sellItemToTrader(
              sessionID,
              info.ItemId,
              info.TraderId,
              info.Price
  ...
  private getItemLowestFleaPrice(templateId: string): number {
    ...
    if (offers && offers.length > 0) {
      offers = offers.filter(
        (a) =>
          a.user.memberType != 4 && //exclude traders
      ...
      if (offers.length > 0)
        return offers.sort((a, b) => a.summaryCost - b.summaryCost)[0]
          .summaryCost;
    }
  ...
  private sellItemToTrader(
    ...
    let response: IItemEventRouterResponse = this.tradeHelper.sellItem(
      pmcData,
      pmcData,
      sellRequest,
      sessionId
