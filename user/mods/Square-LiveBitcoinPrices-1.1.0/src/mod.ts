...
class Mod implements IPostDBLoadModAsync {
  public async postDBLoadAsync(container: DependencyContainer): Promise<void> {
    ...
    const therapist_coef =
      (100 -
        tables.traders["54cb57776803fa99248b456e"].base.loyaltyLevels[0]
          .buy_price_coef) /
      100;
    const bitcoin = handbook.Items.find(
        ...
        (res) => {
          ...
          res.on("end", () => {
            try {
              ...
              const price = parsedData.data.item.sellFor.find(
                (x) => x.vendor.name === "Therapist"
              ).priceRUB;
              resolve(price);
            } catch (e) {
