...
class VCQL implements IPostDBLoadMod, IPreAkiLoadMod {
  ...
  public loadZones() {
    let zones = [];
    this.loadFiles(
      `${modPath}/database/zones/`,
      [".json"],
      function (filePath) {
        const zoneFile = require(filePath);
        if (Object.keys(zoneFile).length > 0) zones.push(...zoneFile);
      }
    );
    this.zones = zones;
  }
  ...
  public importQuests(database, logger, config) {
      ...
      function (filePath) {
        ...
        for (const quest in item) {
          ...
          if (item[quest].startMonth && item[quest].startMonth > 0) {
            ...
            let questStartDate = new Date(
              currentDate.getFullYear(),
              item[quest].startMonth - 1,
              item[quest].startDay
            );
            let questEndDate = new Date(
              currentDate.getFullYear(),
              item[quest].endMonth - 1,
              item[quest].endDay
            );
            ...
            if (currentDate < questStartDate || currentDate > questEndDate) {
              if (debugLogging)
                logger.success(
                  `[VCQL-DEBUG] Removing quest ${item[quest]._id} because it is outside the date range`
                );
              prunedCount++;
              continue;
            }
          ...
          if (item[quest].sideExclusive) {
            if (item[quest].sideExclusive == "Usec" && !ignoreSideExclusive) {
              ...
              if (debugLogging)
                logger.success(
                  `[VCQL-DEBUG] Adding quest ${item[quest]._id} as a Usec exclusive quest.`
                );
            }
            if (item[quest].sideExclusive == "Bear" && !ignoreSideExclusive) {
              ...
              if (debugLogging)
                logger.success(
                  `[VCQL-DEBUG] Adding quest ${item[quest]._id} as a Bear exclusive quest.`
                );
            }
    ...
    if (this.enableLogging) {
      ...
      logger.success(
        `[VCQL] ${prunedCount} custom quests were pruned due to date settings.`
      );
    }
  }
  ...
  public importAssorts(database, logger) {
    let debugLogging = this.enableDebugLogging;
    this.loadFiles(
      `${modPath}/database/assorts/`,
      [".json"],
      function (filePath) {
        const assorts = require(filePath);
        if (
          assorts.items == undefined ||
          assorts.traderID == undefined ||
          assorts.barter_scheme == undefined ||
          assorts.loyal_level_items == undefined
        )
          return;
        ...
        for (const assort of assorts.items) {
          if (assorts.barter_scheme[assort._id] != undefined) {
            databaseTrader.assort.barter_scheme[assort._id] =
              assorts.barter_scheme[assort._id];
          } else {
            if (assort.parentId == "hideout")
              logger.error(
                `[VCQL] Parent assort ${assort._id} has no associated barter scheme.`
              );
          }

          if (assorts.loyal_level_items[assort._id] != undefined) {
            databaseTrader.assort.loyal_level_items[assort._id] =
              assorts.loyal_level_items[assort._id];
          } else {
            if (assort.parentId == "hideout")
              logger.error(
                `[VCQL] Parent assort ${assort._id} has no associated loyalty level`
              );
          }

          if (assort.unlockedOn != undefined && assort.questID != undefined) {
            databaseTrader.questassort[assort.unlockedOn][assort._id] =
              assort.questID;
          ...
          if (debugLogging)
            logger.success(
              `[VCQL-DEBUG] Adding assort ${assort._id} to the trader ${traderID}`
            );
        }
      }
    );
    ...
    for (const locale of serverLocales) {
      ...
      for (const englishItem in addedLocales["en"]) {
        ...
        if (database.locales.global[locale] != undefined)
          database.locales.global[locale][englishItem] =
            addedLocales["en"][englishItem];
      }
    }
  }
  ...
  public routeImages(imageRouter, logger) {
    ...
    this.loadFiles(
      ...
      function (filePath) {
        ...
        if (debugLogging)
          logger.success(
            `[VCQL-DEBUG] Adding quest image ${path.basename(filePath)}`
          );
        questImageCount++;
      }
    );
    ...
    this.loadFiles(
      ...
      function (filePath) {
        imageRouter.addRoute(
          `/files/trader/avatar/${path.basename(
            filePath,
            path.extname(filePath)
          )}`,
          filePath
