...
class RemotePlugins implements IPreAkiLoadMod, IPostAkiLoadMod, IPostDBLoadMod {
  ...
  public preAkiLoad(container: DependencyContainer): void {
    ...
    httpListenerService.registerHttpListener(
      "RemotePluginsClientOptionsHttpListener",
      this.remotePluginsClientOptionsHttpListener_canHandleOverride,
      this.remotePluginsClientOptionsHttpListener_handleOverride()
    );
    httpListenerService.registerHttpListener(
      "RemotePluginsFileMapHttpListener",
      this.remotePluginsFileMapHttpListener_canHandleOverride,
      this.remotePluginsFileMapHttpListener_handleOverride()
    );
    httpListenerService.registerHttpListener(
      "RemotePluginsFileHttpListener",
      this.remotePluginsFileHttpListener_canHandleOverride,
      this.remotePluginsFileHttpListener_handleOverride()
    );
  }
    ...
    try {
      ...
      if (fs.existsSync(`${this.rootPluginPath}/config/config.json`)) {
        const configReader = fs.readFileSync(
          `${this.rootPluginPath}/config/config.json`,
          "utf8"
        );
        this.modConfig = {
          ...this.modConfig,
          ...(JSON.parse(configReader) as ModConfig),
        };
      }
      // this is to ensure that the config is always up to date
      fs.writeFileSync(
        `${this.rootPluginPath}/config/config.json`,
        JSON.stringify(this.modConfig, null, 4)
      );
    } catch (e) {
  ...
  public remotePluginsClientOptionsHttpListener_canHandleOverride(
    sessionId: string,
    req: IncomingMessage
  ): boolean {
    return (
      req.method === "GET" && req.url?.includes(`/${modName}/ClientOptions`)
    );
  }

  public remotePluginsClientOptionsHttpListener_handleOverride(): (
    sessionId: string,
    req: IncomingMessage,
    resp: ServerResponse
  ) => void {
  ...
  public remotePluginsFileMapHttpListener_canHandleOverride(
    sessionId: string,
    req: IncomingMessage
  ): boolean {
  ...
  public remotePluginsFileMapHttpListener_handleOverride(): (
    sessionId: string,
    req: IncomingMessage,
    resp: ServerResponse
  ) => void {
    ...
    const responseFunc = async (
      ...
      let retryCount = 0;
      while (!_this.cachedPluginFileMapJson && retryCount < 60) {
        // 30 seconds
  ...
  public remotePluginsFileHttpListener_canHandleOverride(
    sessionId: string,
    req: IncomingMessage
  ): boolean {
  ...
  public remotePluginsFileHttpListener_handleOverride(): (
    sessionId: string,
    req: IncomingMessage,
    resp: ServerResponse
  ) => void {
    ...
    const responseFunc = async (
      ...
      let retryCount = 0;
      while (!_this.cachedPluginFileMap && retryCount < 60) {
        // 30 seconds
        await _this.sleep(500);
        retryCount++;
      }
      ...
      if (
        _this.cachedPluginFileMap.Zip.Name !== "bepinex.zip" ||
        !_this.cachedPluginFileMap.Zip.Hash ||
        !_this.cachedPluginFileMap.Zip.Size
      ) {
        resp.writeHead(500, "Failed to get file");
        resp.end();
        return;
  ...
  public async postAkiLoad(container: DependencyContainer): Promise<void> {
    ...
    this.fileWatcher = chokidar.watch(
      [this.bepinexFilePath, this.storagePath],
      {
        persistent: true,
        ignoreInitial: true,
        depth: 5,
    ...
    try {
      if (this.cachedPluginFileMap.Zip) {
        const bepinexHash = this.generateFileHash(
          `${this.storagePath}bepinex.zip`
        );
  ...
  private async createNewData(writeFiles = true): Promise<PluginFileMap> {
    ...
    for (const file of files) {
      ...
      if (fs.lstatSync(file).isDirectory()) {
        ...
        if (writeFiles) {
          ...
          }); // level 1 is fastest and in my testing the time/compression ratio doesn't get better with higher levels
        }
      }
    }
    ...
    if (Object.keys(pluginFileMap.Files).length === 0) {
      ...
      try {
        this.vfs.writeFile(
          `${this.storagePath}fileMap.json`,
          JSON.stringify(pluginFileMap),
          false,
          true
        );
    ...
    // generate hashes
    pluginFileMap.FilesHash = this.hashUtil.generateHashForData(
      "sha256",
      JSON.stringify(pluginFileMap.Files)
    );
    if (!writeFiles) {
      return pluginFileMap;
    }
  ...
  private async FilesDidChange(fileMap: PluginFileMap): Promise<boolean> {
    ...
    if (currentFileMap.FilesHash !== fileMap.FilesHash) {
      const hash =
        currentFileMap.FilesHash === null ||
        currentFileMap.FilesHash.length === 0
          ? "empty"
          : currentFileMap.FilesHash;
