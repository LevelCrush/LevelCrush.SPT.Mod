...
export class BundleLoaderOverride {
  ...
  public addBundles(modpath: string): void {
    ...
    for (const bundleManifest of bundleManifestArr) {
      ...
      const absoluteModPath = path
        .join(process.cwd(), modpath)
        .slice(0, -1)
        .replace(/\\/g, "/");
      const bundleLocalPath = `${modpath}bundles/${bundleManifest.key}`.replace(
        /\\/g,
        "/"
      );
      ...
      const bundleHash =
        this.bundleHashCacheService.getStoredValue(bundleLocalPath);

      this.addBundle(
        bundleManifest.key,
        new BundleInfo(absoluteModPath, bundleManifest, bundleHash)
      );
    }
  }
