...
export class StayInTarkovMod implements IPreAkiLoadMod, IPostDBLoadMod {
  ...
  private InitializeVariables(container: DependencyContainer): void {
    ...
    this.bundleCallbacks =
      container.resolve<BundleCallbacks>("BundleCallbacks");
    this.inraidCallbacks =
      container.resolve<InraidCallbacks>("InraidCallbacks");
    ...
    this.profileCallbacks =
      container.resolve<ProfileCallbacks>("ProfileCallbacks");
  ...
  public preAkiLoad(container: DependencyContainer): void {
    ...
    const launcherControllerOverride = new LauncherControllerOverride(
      container,
        ...
        new RouteAction(
          ...
          (url: string, info: any, sessionID: string, output: string): any => {
            ...
            if (coopMatch === undefined) {
              console.error(
                `Cannot retrieve LocationData for unknown ServerId: ${info.serverId}!`
              );
            ...
            if (coopMatch.LocationData === undefined) {
              console.error(
                `No LocationData found in server ${coopMatch.ServerId}!`
              );
              return this.httpResponse.nullResponse();
            }
        ...
        new RouteAction(
          ...
          (url: string, info: any, sessionID: string, output: string): any => {
            ...
            if (matchId !== undefined) {
              ...
              if (coopMatch !== undefined) spawnPoint = coopMatch.SpawnPoint;
            }
        ...
        new RouteAction(
          ...
          (url: string, info: any, sessionID: string, output: string): any => {
            ...
            if (matchId !== undefined) {
              // console.log("matchId:" + matchId);
              const coopMatch = this.getCoopMatch(matchId);
              if (coopMatch !== undefined) friendlyAI = coopMatch.friendlyAI;
            }
      ...
      [
        {
          ...
          action: (url, info: any, sessionId: string, output) => {
            ...
            for (let itemKey in CoopMatch.CoopMatches) {
              ...
              if (
                info.side.toLowerCase() != "savage" &&
                m.Side.toLowerCase() == "savage"
              )
                continue;
              ...
              if (m.Time != info.timeVariant) continue;

              // Filter out Raids that are in a different location
              if (m.Location != info.location) continue;
              ...
              // Passworded
              matchResponse.IsPasswordLocked =
                m.Password !== undefined && m.Password !== "";
              // Game Version
        ...
        },
        {
          ...
          action: (url, info, sessionId, output) => {
            ...
            for (let cm in CoopMatch.CoopMatches) {
              ...
              if (CoopMatch.CoopMatches[cm].Location != info.location) continue;

              if (CoopMatch.CoopMatches[cm].Time != info.timeVariant) continue;
              ...
              if (
                CoopMatch.CoopMatches[cm].LastUpdateDateTime <
        ...
        },
        {
          ...
          action: (url, info, sessionId, output) => {
            ...
            if (
              coopMatch.ConnectedUsers.findIndex((x) => x == info.profileId) !==
              -1
            ) {
              if (
                WebSocketHandler.Instance.webSockets[info.profileId] !==
                undefined
              ) {
                if (
                  WebSocketHandler.Instance.webSockets[info.profileId]
                  ...
                  logger.info(
                    `JoinMatch failed: ${info.profileId} is already connected!`
                  );
            ...
            if (
              coopMatch.AuthorizedUsers.findIndex(
                (x) => x == info.profileId
              ) === -1
            ) {
              coopMatch.AuthorizedUsers.push(info.profileId);
              logger.info(
                `Added authorized user: ${info.profileId} in server: ${coopMatch.ServerId}`
              );
            }
          ...
          action: (url, info, sessionId, output) => {
            ...
            if (CoopMatch.CoopMatches[info.serverId] !== undefined) {
              if (info.serverId == sessionId) {
                ...
                logger.success(
                  `Successfully deleted Coop Server ${info.serverId}`
                );
              } else {
            ...
            switch (response.response) {
              case "UNAUTHORIZED":
                logger.warning(
                  `Request to delete Coop Server ${info.serverId} denied`
                );
                break;
              case "NOT_EXIST":
                logger.debug(
                  `Request to delete Coop Server ${info.serverId}. Server doesn't exist.`
                );
                break;
            }
      ...
      [
        ...
        },
        {
          ...
          action: (
            ...
            return StayInTarkovMod.Instance.gameCallbacks.gameStart(
              url,
              info,
              sessionID
            );
        ...
        },
        {
          ...
          action: (
            ...
            const profileC =
              StayInTarkovMod.Instance.profileCallbacks.createProfile(
                url,
                info,
                sessionID
              );
  ...
  postDBLoad(container: DependencyContainer): void {
    ...
    const dbTables = StayInTarkovMod.container
      .resolve<DatabaseServer>("DatabaseServer")
      .getTables();
    dbTables.locales.global["en"][
  ...
  private updateExtracts(locations: any): void {
    ...
    for (const location of locationNames) {
      ...
      for (const extract in locations[location].base.exits) {
        ...
        if (
          locations[location].base.exits[extract].PassageRequirement === "Train"
        ) {
          continue;
        }
