...
export class WebSocketHandler {
  ...
  constructor(webSocketPort: number, logger: ILogger) {
  ...
  private async processMessageString(msg: RawData, msgStr: string) {
    ...
    if (msgStr.startsWith("SIT")) {
      ...
      const messageWithoutSITPrefixes = messageWithoutSITPrefix.substring(
        24,
        messageWithoutSITPrefix.length
      );

      const match = CoopMatch.CoopMatches[serverId];
      if (match !== undefined) {
        // match.ProcessData(messageWithoutSITPrefixes, this.logger);
        const method = messageWithoutSITPrefixes.substring(
          1,
          messageWithoutSITPrefixes.indexOf("?")
        );
        let d = messageWithoutSITPrefixes
          .substring(messageWithoutSITPrefixes.indexOf("?"))
          .replace("?", "");
        ...
        WebSocketHandler.Instance.sendToWebSockets(
    ...
    for (let session of sessions) {
      if (this.webSockets[session] !== undefined) {
        if (this.webSockets[session].readyState === WebSocket.OPEN) {
          if (data !== undefined) this.webSockets[session].send(data);
  ...
  public closeWebSocketSession(session: string, reason: string) {
    if (this.webSockets[session] !== undefined) {
      if (this.webSockets[session].readyState === WebSocket.OPEN) {
        this.webSockets[session].send(
          JSON.stringify({ endSession: true, reason: reason })
        );
