...
export class SITHelpers {
  /**
     ...
     * @returns 
     */
  public fixProfileEquipmentId(
    container: tsyringe.DependencyContainer,
    sessionID: string
  ) {
    ...
    if (myProfile.characters.pmc.Inventory === undefined) return;
    ...
    const resultString = inventoryString.replace(
      new RegExp(equipmentId, "g"),
      newEquipmentId
    );
