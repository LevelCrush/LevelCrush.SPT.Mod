...
export class UPNPHelper {
  ...
  doUPNPMappingsOwn(): void {
    const client = new ownUPNP({
      enablePMP: true,
      enableUPNP: true,
      permanentFallback: true,
    });
    const akiPromise = client.portMapping({
      public: this.akiPort,
      private: this.akiPort,
      protocol: "TCP",
    });
    const sitWSPromise = client.portMapping({
      public: this.coopConfig.webSocketPort,
      private: this.coopConfig.webSocketPort,
      protocol: "TCP",
    });
    const natPromise = client.portMapping({
      public: this.coopConfig.natHelperPort,
      private: this.coopConfig.natHelperPort,
      protocol: "TCP",
    });
    const p2pPromise = client.portMapping({
      public: 6972,
      private: 6972,
      protocol: "UDP",
    });

    Promise.all(new Array(akiPromise, sitWSPromise, natPromise, p2pPromise))
      .then((x) => {
        console.log(
          `SIT: UPNP: Successfully mapped ${this.akiPort},${this.coopConfig.webSocketPort},${this.coopConfig.natHelperPort},6972`
        );
      })
      .catch((rejectedReason) => {
        if (
          rejectedReason instanceof Error &&
          rejectedReason.message.includes("Timeout")
        ) {
          console.log(
