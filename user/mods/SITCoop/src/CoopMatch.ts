...
export class CoopMatch {
  ...
  public constructor(inData: any) {
    ...
    if (inData.settings === undefined) return;
  ...
  public ProcessData(info: any, logger: ILogger) {
    ...
    if (info.profileId !== undefined) this.PlayerJoined(info.profileId);
    ...
    if (info.m == "PlayerSpawn") {
      ...
      if (!foundExistingPlayer) this.Characters.push(info);
    }
  ...
  public PlayerJoined(profileId: string) {
    ...
    if (this.ConnectedUsers.findIndex((x) => x == profileId) === -1) {
      ...
      if (this.AuthorizedUsers.findIndex((x) => x == profileId) === -1) {
        console.log(
          `${profileId} is not authorized in server: ${this.ServerId}`
        );

        WebSocketHandler.Instance.closeWebSocketSession(
          profileId,
          CoopMatchEndSessionMessages.WEBSOCKET_TIMEOUT_MESSAGE
        );
