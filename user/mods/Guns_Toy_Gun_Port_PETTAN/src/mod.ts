...
class MFACitems implements IPostDBLoadMod {
  ...
  public postDBLoad(container: DependencyContainer): void {
    ...
    //Mod Info
    const modFolderName = "Guns_Toy_Gun_Port_PETTAN";
    const modFullName = "Guns_Toy_Gun_Port_PETTAN";
    ...
    this.mydb = databaseImporter.loadRecursive(
      `${modLoader.getModPath(modFolderName)}database/`
    );
    ...
    const newBlacklist2 = serverScavcaseConfig.rewardItemBlacklist.concat(
      blacklist.addtoconfigsscavcase.rewardItemBlacklist
    );
      ...
      if (mmFACItem.enable) {
        if ("clone" in mmFACItem) {
          ...
          this.copyToFilters(
            mmFACItem.clone,
            mmFACID,
            mmFACItem.enableCloneCompats,
            mmFACItem.enableCloneConflicts
          );
    ...
    for (const mmFACID in this.mydb.mmfac_items)
      if (this.mydb.mmfac_items[mmFACID].enable) this.addToFilters(mmFACID);
    this.logger.debug(modFolderName + " item filters finished");

    //Clothing
    for (const [mmFACID, mmfacArticle] of Object.entries(
      this.mydb.mmfac_clothes
    )) {
      //Articles + Handbook
    ...
    for (const preset in this.mydb.globals.ItemPresets)
      this.db.globals.ItemPresets[preset] =
        this.mydb.globals.ItemPresets[preset];
    ...
    for (const weapon in dbMastering) {
      if (dbMastering[weapon].Name == "PM")
        dbMastering[weapon].Templates.push("66015072e9f84d5680039678");
    }
  ...
  private cloneItem(itemToClone: string, mmFACID: string): void {
    ...
    if (this.mydb.mmfac_items[mmFACID].enable == true) {
      ...
      let mmFACItemOut = this.jsonUtil.clone(
        this.db.templates.items[itemToClone]
      );

      //Change the necessary item attributes using the info in our database file mmfac_items.json
      mmFACItemOut._id = mmFACID;
      mmFACItemOut = this.compareAndReplace(
        mmFACItemOut,
        this.mydb.mmfac_items[mmFACID]["item"]
      );
  ...
  private createItem(itemToCreate: string): void {
    ...
    if (newItem.enable) {
      ...
      this.logger.debug(
        "Item " + itemToCreate + " created and added to database."
      );
  ...
  private checkItem(itemToCheck: ImmFACItem): [boolean, ITemplateItem] {
    ...
    for (const level1 in itemTemplate) {
      if (!(level1 in itemToCheck.item)) {
        this.logger.error(
          'ERROR - Missing attribute: "' + level1 + '" in your item entry!'
        );
        pass = false;
      }
    }
    ...
    for (const prop in itemToCheck.item._props) {
      if (!(prop in itemTemplate._props))
        this.logger.warning(
          'WARNING - Attribute: "' + prop + '" not found in item template!'
        );
    }
    ...
    for (const key in attributesToChange) {
      ...
      if (
        ["boolean", "string", "number"].includes(
          typeof attributesToChange[key]
        ) ||
        Array.isArray(attributesToChange[key])
      ) {
        ...
        //TO DO: Add check with item template here if someone wants to add new properties to a cloned item.
          ...
          this.logger.warning(
            "(Item: " +
              originalItem._id +
              ') WARNING: Could not find the attribute: "' +
              key +
              '" in the original item, make sure this is intended!'
          );
      ...
      //Otherwise keep traveling down the nest
        ...
        originalItem[key] = this.compareAndReplace(
          originalItem[key],
          attributesToChange[key]
        );
    }
  ...
  private getFilters(item: string): [Array<Slot>, Array<string>] {
    ...
    const slots =
      typeof this.db.templates.items[item]._props.Slots === "undefined"
        ? []
        : this.db.templates.items[item]._props.Slots;
    const chambers =
      typeof this.db.templates.items[item]._props.Chambers === "undefined"
        ? []
        : this.db.templates.items[item]._props.Chambers;
    const cartridges =
      typeof this.db.templates.items[item]._props.Cartridges === "undefined"
        ? []
        : this.db.templates.items[item]._props.Cartridges;
    const filters = slots.concat(chambers, cartridges);

    const conflictingItems =
      typeof this.db.templates.items[item]._props.ConflictingItems ===
      "undefined"
        ? []
        : this.db.templates.items[item]._props.ConflictingItems;

    return [filters, conflictingItems];
  }

  private copyToFilters(
    itemClone: string,
    mmFACID: string,
    enableCompats = true,
    ...
    for (const item in this.db.templates.items) {
      ...
      if (enableConflicts)
        for (const conflictID of conflictingItems)
          if (conflictID === itemClone) conflictingItems.push(mmFACID);
    }
  }
    ...
    if (mmfacNewItem.enable) {
      ...
      if ("addToThisItemsFilters" in mmfacNewItem) {
        ...
        for (const modSlotName in mmfacNewItem.addToThisItemsFilters) {
          if (modSlotName === "conflicts")
            mmFACConflictingItems = mmFACConflictingItems.concat(
              mmfacNewItem.addToThisItemsFilters.conflicts
            );
          else {
            for (const filter in mmFACItemFilters) {
              if (modSlotName === mmFACItemFilters[filter]._name) {
                const slotFilter =
                  mmFACItemFilters[filter]._props.filters[0].Filter;
                const newFilter = slotFilter.concat(
                  mmfacNewItem.addToThisItemsFilters[modSlotName]
                );
      ...
      if ("addToExistingItemFilters" in mmfacNewItem) {
        for (const modSlotName in mmfacNewItem.addToExistingItemFilters) {
          if (modSlotName === "conflicts") {
            for (const conflictingItem of mmfacNewItem.addToExistingItemFilters[
              modSlotName
            ]) {
            ...
            for (const compatibleItem of mmfacNewItem.addToExistingItemFilters[
              modSlotName
            ]) {
  ...
  private cloneClothing(articleToClone: string, mmFACID: string): void {
    if (
      this.mydb.mmfac_clothes[mmFACID].enable ||
      !("enable" in this.mydb.mmfac_clothes[mmFACID])
    ) {
      //Get a clone of the original item from the database
      let mmfacClothingOut = this.jsonUtil.clone(
        this.db.templates.customization[articleToClone]
      );
      ...
      mmfacClothingOut = this.compareAndReplace(
        mmfacClothingOut,
        this.mydb.mmfac_clothes[mmFACID]["customization"]
      );
  ...
  private createClothing(articleToCreate: string): void {
    ...
    if (newArticle.enable) {
      ...
      this.logger.debug(
        "Article " + articleToCreate + " created and added to database."
      );
    }
  }

  private checkArticle(
    articleToCheck: ImmFACCustomizationItem
  ): [boolean, ICustomizationItem] {
    ...
    for (const level1 in articleTemplate) {
      if (!(level1 in articleToCheck.customization)) {
        this.logger.error(
          'ERROR - Missing attribute: "' + level1 + '" in your article entry!'
        );
        pass = false;
      }
    }
    ...
    for (const prop in articleToCheck.customization._props) {
      if (!(prop in articleTemplate._props))
        this.logger.warning(
          'WARNING - Attribute: "' + prop + '" not found in article template!'
        );
    }
  ...
  private addTraderAssort(trader: string): void {
    ...
    for (const item in this.mydb.traders[trader].assort.items) {
      ...
      this.db.traders[trader].assort.items.push(
        this.mydb.traders[trader].assort.items[item]
      );
    }
    ...
    for (const item in this.mydb.traders[trader].assort.barter_scheme) {
      //this.logger.debug(item + " added to " + trader);
      this.db.traders[trader].assort.barter_scheme[item] =
        this.mydb.traders[trader].assort.barter_scheme[item];
    }
    ...
    for (const item in this.mydb.traders[trader].assort.loyal_level_items) {
      ...
      if (modConfig.lvl1Traders)
        ...
        this.db.traders[trader].assort.loyal_level_items[item] =
          this.mydb.traders[trader].assort.loyal_level_items[item];
    }
  }
  ...
  private addLocales(
    mmFACID: string,
    mmFACItem?: ImmFACItem,
    mmfacArticle?: ImmFACCustomizationItem
  ): void {
    ...
    for (const localeID in this.db.locales.global) {
      //For each possible locale/language in SPT's database
      let localeEntry: ImmFACLocale;

      if (mmfacEntry.locales) {
        if (localeID in mmfacEntry.locales) {
          //If the language is entered in mmfac_items, use that
          localeEntry = {
        ...
        } //Otherwise use english as the default
        else {
          localeEntry = {
        ...
        //Normal
          ...
          this.db.locales.global[localeID][description] =
            localeEntry.Description;
        }
      } else {
        if (isItem)
          this.logger.warning(
            "WARNING: Missing locale entry for item: " + mmFACID
          );
        else
      ...
      if (isItem && mmFACItem.presets) {
        for (const preset in mmFACItem.presets) {
          if (modConfig.oldLocales)
            ...
            this.db.locales.global[localeID][preset] =
              mmFACItem.presets[preset];
          }
        }
      }
    }
