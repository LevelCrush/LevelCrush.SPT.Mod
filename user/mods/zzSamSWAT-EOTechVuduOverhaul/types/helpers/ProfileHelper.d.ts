...
export declare class ProfileHelper {
  ...
  removeCompletedQuestConditionFromProfile(
    pmcData: IPmcData,
    questConditionId: Record<string, string>
  ): void;
  /**
   ...
   */
  isNicknameTaken(
    nicknameRequest: IValidateNicknameRequestData,
    sessionID: string
  ): boolean;
  protected profileHasInfoProperty(profile: IAkiProfile): boolean;
  protected nicknameMatches(
    profileName: string,
    nicknameRequest: string
  ): boolean;
  protected sessionIdMatchesProfileId(
    profileId: string,
    sessionId: string
  ): boolean;
  /**
  ...
  incrementStatCounter(
    counters: CounterKeyValue[],
    keyToIncrement: string
  ): void;
  /**
