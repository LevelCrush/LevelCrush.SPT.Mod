  ...
  public validateCustomQuests(): void {
    ...
    if (this.vfs.exists(`${path}/standard/`)) {
      ...
      for (const i in standardQuests) {
        const questFileText = this.vfs.readFile(
          `${path}/standard/${standardQuests[i]}`
        );
        ...
        this.commonUtils.logInfo(
          `Found ${quests.length} standard quest(s) in "/standard/${standardQuests[i]}"`
        );
      }

      if (totalStandardQuestsAdded === 0) {
        this.commonUtils.logError(
          "No standard quests have been added. Mod files may have been corrupted. Please try reinstalling."
        );
      }
    } else {
      this.commonUtils.logError(
        'The "user\\mods\\DanW-SPTQuestingBots\\quests\\standard" directory is missing. Mod files may have been corrupted. Please try reinstalling.'
      );
    }
    ...
    if (this.vfs.exists(`${path}/custom/`)) {
      ...
      for (const i in customQuests) {
        ...
        this.commonUtils.logInfo(
          `Found ${quests.length} custom quest(s) in "/custom/${customQuests[i]}"`
        );
      }
