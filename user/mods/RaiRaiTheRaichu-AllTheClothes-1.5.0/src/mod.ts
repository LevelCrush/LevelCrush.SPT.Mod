...
class AllTheClothes implements IPostDBLoadMod {
  ...
  public postDBLoad(container: DependencyContainer): void {
    ...
    var clothingBlacklist = [
      "DefaultUsecBody", // These are already unlockable via other means.
      "usec_upper_acu",
      "usec_upper_commando",
    ...
    var handBundles = {
      hands_boss_big_pipe: "assets/hands_boss_big_pipe.skin.bundle",
      ...
      hands_scav_wildbody:
        "assets/content/hands/wild/wild_body_firsthands.bundle",
    ...
    for (var upperClothing in databaseServer.getTables().globals.config
      .Customization.SavageBody) {
      ...
      let newKit = {
        ...
        _props: {
          ...
          Body: databaseServer.getTables().globals.config.Customization
            .SavageBody[upperClothing].body,
          Hands:
            this.config[upperClothing]?.handsOverride ??
    ...
    for (var lowerClothing in databaseServer.getTables().globals.config
      .Customization.SavageFeet) {
      ...
      let newSuite = {
        ...
        _props: {
          ...
          Side: ["Usec", "Bear", "Savage"],
          AvailableAsDefault: false,
          Feet: databaseServer.getTables().globals.config.Customization
            .SavageFeet[lowerClothing].feet,
        },
      };
    ...
    if (this.config.AllPMCClothesFree) {
      for (let suitOffer in databaseServer.getTables().traders[
        "5ac3b934156ae10c4430e83c"
      ].suits) {
        databaseServer.getTables().traders["5ac3b934156ae10c4430e83c"].suits[
          suitOffer
        ].requirements = {
          loyaltyLevel: 0,
          profileLevel: 0,
    ...
    for (var clothing in clothingToAdd) {
      ...
      let newTraderOffer = {
        ...
        tid: this.config.UseRagmanInsteadOfFence
          ? "5ac3b934156ae10c4430e83c"
          : this.config[configName]?.ragman
          ? "5ac3b934156ae10c4430e83c"
          : "579dc571d53a0658a154fbec",
        suiteId: clothingToAdd[clothing]._id,
        isActive: true,
        requirements: {
          loyaltyLevel: this.config.AllScavClothesFree
            ? 0
            : this.config[configName].loyaltyLevel,
          profileLevel: this.config.AllScavClothesFree
            ? 0
            : this.config[configName].profileLevel,
          standing: this.config.AllScavClothesFree
            ? 0
            : this.config[configName].standing,
          skillRequirements: this.config.AllScavClothesFree
            ? []
            : this.config[configName].skillRequirements,
          questRequirements: this.config.AllScavClothesFree
            ? []
            : this.config[configName].questRequirements,
          itemRequirements: this.config.AllScavClothesFree
            ? []
            : this.config[configName].itemRequirements,
        },
      };
      ...
      databaseServer.getTables().templates.customization[
        clothingToAdd[clothing]._id
      ] = clothingToAdd[clothing];
    }

    // Make sure Fence can sell clothes.
    if (databaseServer.getTables().traders["579dc571d53a0658a154fbec"].suits) {
      let fenceSuits =
        databaseServer.getTables().traders["579dc571d53a0658a154fbec"].suits;
      fenceSuits.push(...fenceOffers);
      databaseServer.getTables().traders["579dc571d53a0658a154fbec"].suits =
        fenceSuits;
    } else {
      databaseServer.getTables().traders[
        "579dc571d53a0658a154fbec"
      ].base.customization_seller = true;
      databaseServer.getTables().traders["579dc571d53a0658a154fbec"].suits =
        fenceOffers;
    }
    ...
    for (const customizationItem in databaseServer.getTables().templates
      .customization) {
      if (
        databaseServer.getTables().templates.customization[customizationItem]
          ._parent === "5cc085e214c02e000c6bea67" &&
        databaseServer
          .getTables()
          .templates.customization[customizationItem]._props.Side.includes(
            "Usec"
          ) &&
        !databaseServer
          .getTables()
        ...
        databaseServer
          .getTables()
          .templates.customization[customizationItem]._props.Side.push("Bear");
        continue;
      }
      if (
        databaseServer.getTables().templates.customization[customizationItem]
        ...
        databaseServer
          .getTables()
          .templates.customization[customizationItem]._props.Side.push("Usec");
        continue;
      }
    }
    ...
    for (const customizationItem in databaseServer.getTables().templates
      ...
      if (
        ...
        databaseServer.getTables().templates.customization[
          customizationItem
        ]._props.Side = ["Usec", "Bear", "Savage"];
        if (
          !databaseServer
            .getTables()
            .templates.character.includes(customizationItem)
        ) {
          databaseServer
            .getTables()
            .templates.character.push(customizationItem);
        }
        continue;
      }
    ...
    if (this.config.UnlockAllHeads === true) {
      for (let head in headList) {
        databaseServer.getTables().templates.customization[headList[head]._id] =
          headList[head];
    ...
    for (const localeID in databaseServer.getTables().locales.global) {
      ...
      for (let entryType in localeEntryTypes) {
        delete databaseServer.getTables().locales.global[localeID][
          `6287b0d239d8207cb27d66c7 ${localeEntryTypes[entryType]}`
        ];
        delete databaseServer.getTables().locales.global[localeID][
          `628b57d800f171376e7b2634 ${localeEntryTypes[entryType]}`
        ];
        delete databaseServer.getTables().locales.global[localeID][
          `628b57d800f171376e7b2634 ${localeEntryTypes[entryType]}`
        ];
        delete databaseServer.getTables().locales.global[localeID][
          `5fc615710b735e7b024c76ed ${localeEntryTypes[entryType]}`
        ];
        delete databaseServer.getTables().locales.global[localeID][
          `62875ad50828252c7a28b95c ${localeEntryTypes[entryType]}`
        ];
      }

      for (let template in modLocale) {
        databaseServer.getTables().locales.global[localeID][
          `${template} Name`
        ] = modLocale[template].Name;
        databaseServer.getTables().locales.global[localeID][
          `${template} ShortName`
        ] = modLocale[template].ShortName;
        databaseServer.getTables().locales.global[localeID][
          `${template} Description`
        ] = modLocale[template].Description;
      }
    }
    ...
    if (this.config.UnlockFactionalClothing === true) {
      for (let clothing in databaseServer.getTables().templates.customization) {
        let parent =
          databaseServer.getTables().templates.customization[clothing]._parent;
